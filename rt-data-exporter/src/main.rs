...
async fn main() {
    ...
    let mut wtr = Writer::from_path("rt-data.csv").expect("CSV Writer: Could not open file.");

    loop {
        tokio::select! {
            Some(msg) = receiver.recv() => {
                let vehicle: Vehicle = serde_json::from_str(&msg).expect("Serde: Error deserializing vehicle.");
                let csv_vehicle = make_vehicle_serializable(vehicle);
                wtr.serialize(csv_vehicle).expect("CSV Writer: Error serializing vehicle to csv.");
            ...
            () = &mut sleep => {
                wtr.flush().expect("CSV Writer: Error while flushing to disk");
                println!("{}: flushed to disc after {}ms of inactivity", Utc::now(), TIMEOUT);
...
async fn redis_subscriber(sender: UnboundedSender<String>) {
    ...
    let _: () = redis_conn.subscribe(&["realtime-with-metadata"], |msg| {
        let data: String = msg.get_payload().unwrap();
        sender.send(data).expect("Internal channel broken");
